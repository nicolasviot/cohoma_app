...
//callback for robot_state msg (contains data on one robot)
  ...
  RosNodeProxy::receive_msg_robot_state(const icare_interfaces::msg::RobotState::SharedPtr msg) {
    ...
    SET_CHILD_DOUBLE (robot, "lat", msg->position.latitude, true);
    SET_CHILD_DOUBLE (robot, "lon", msg->position.longitude, true);
    SET_CHILD_DOUBLE (robot, "altitude_msl", msg->position.altitude, true);
    SET_CHILD_DOUBLE (robot, "heading_rot", msg->compass_heading, true);
    SET_CHILD_INT    (robot, "battery_percentage", msg->battery_percentage, true);
    SET_CHILD_INT    (robot, "operation_mode", msg->operating_mode, true); // FIXME: operation_mode vs operating_mode
    SET_CHILD_BOOL   (robot, "emergency_stop", msg->emergency_stop, true);
    SET_CHILD_BOOL   (robot, "failsafe", msg->failsafe, true);
